...
namespace GodhomeEloCounter
{
    ...
    public class LocalData
    {
        ...
        public Boss FindOrCreateBoss(string scene, int tier)
        {
            double baseELO = GodhomeEloCounter.Instance.config.baseELO;

            foreach (Boss boss in bosses)
            {
                if (boss.sceneName == scene && boss.tier == tier)
                {
            ...
            bosses.Add(new Boss(scene, tier) { elo = baseELO, lastElo = baseELO, peakElo = baseELO, streak = 0, bestWinStreak = 0, wins = 0, losses = 0, timeSpent = TimeSpan.Zero });
            return bosses.Last();
        }
    }
